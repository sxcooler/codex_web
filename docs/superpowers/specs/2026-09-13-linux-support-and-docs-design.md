...
- [ ] 实施便携启动安装选择及失效配置恢复，核对官方安装器，测试同意、手动、取消、失败和成功后继续配置分支。
...
- [ ] 检查源码导出文件清单包含 README_EN.md 和新 docs 路径，不包含 WSL 测试数据；因补充需求修改了便携启动流程，生成隔离 Windows 便携产物完成启动验证，正式发布包在最终交付时按用户要求生成。
