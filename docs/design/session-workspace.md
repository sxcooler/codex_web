...
## 会话页固定顶栏与输入区收起（2026-09-22）

**状态：用户已确认，已实施。** 本节以用户提供的会话页标注截图为依据；红框指会话信息区，黄框指输入区右上角入口，绿框指输入区。色框不进入正式界面。本节替代此前相关标题、路径及按钮布局规定，其他流程保持现有行为。验证范围与执行结果见 [实施验收](../verification/2026-09-22-session-toolbar.md)。

### 当前原型图（第四版，操作菜单精简修订）

![固定单行顶栏：项目名与自适应标题、统一操作菜单、标题悬停全文，以及输入区展开和收起示意](../assets/session-toolbar-v4.png)

[查看第四版原图](../assets/session-toolbar-v4.png)。此图替代前三版的顶部设计：上图展示输入区展开及操作菜单，下图展示输入区收起及标题悬停全文。图中剩余空间足够，因此标题直接完整显示；仅在真实窗口不足以容纳全文时由 CSS 隐藏溢出。两种状态都只有同一条固定顶栏，没有顶部展开态，也没有顶部折叠按钮；项目名、标题、操作、运行状态、用量依次排列，两端保留侧栏开关。

图片由内置图像生成工具基于脱敏第三版修订，用于确认布局，不是已实现页面截图。图中的标题、项目名、对话、文件名、进度和用量均为虚构演示数据，不使用真实会话内容。生成约束：顶部固定单行，项目名替代固定品牌文字，操作在运行状态之前；菜单只包含同一视觉样式的“刷新”和“释放”，没有会话信息块；悬停短标题展示完整标题。图片中的底部恢复栏已由下文“紧凑输入栏”修订替代；顶栏布局仍适用。

### 目标与最终修订方向

减少上下固定区域占用，把释放的高度交给中间对话；保留当前深色配色、薄边框、绿色主按钮，不改变左右侧栏和用量入口。标题变短仅是显示省略，不改写原生会话名称；项目不再常驻显示绝对路径。

根据用户最新反馈，取消顶部展开态，始终采用原收起态的单行布局，不再保存顶部折叠偏好。原来的双行摘要、展开标题区及顶部折叠按钮均不采用。底部输入区仍默认展开，可以独立收起；不随滚动、发送、任务完成或新消息自动开合，也不影响左右侧栏及消息内“执行过程”折叠。

### 顶部：固定单行，不再展开

会话页以项目名称替换顶栏左侧固定的“Codex Web”文字；品牌仍由左侧导航保留。顶栏从左至右严格为：**左侧栏开关 → 项目名 → 会话标题 → 操作下拉 → 运行/连接状态 → 用量入口 → 右侧栏开关**。非会话页面的顶栏不在本次修改范围。

顶栏使用现有高度，手机触摸区域不低于 44px。原标题/项目区直接删除，不保留第二行、空白占位或额外边框，对话内容紧接顶栏下沿。没有顶部展开按钮；操作菜单的小箭头只用于打开菜单，不能恢复旧标题区。

- 标题使用全部剩余宽度，**能放下就完整显示，只有放不下才隐藏溢出**。不使用 `slice`/固定字数裁剪，不预先添加省略号，也不设置人为的固定标题宽度或 `max-width`。标题弹性区使用 `flex: 1 1 0; min-width: 0; overflow: hidden; white-space: nowrap; text-overflow: ellipsis`，右侧操作、状态、用量及侧栏按钮保持自身宽度。**鼠标悬停标题时显示原始完整标题**，优先使用原生 `title` 提示，不新增 tooltip 依赖。完整标题保留在 DOM 文本和可访问名称中，不在“操作”内重复展示。标题本身不作为展开或导航按钮。
- 项目使用项目显示名；缺少名称时取目录末级名称，同时支持 Windows 和 Linux 分隔符。无可用项目时显示“未关联项目”，不把会话 ID 当项目名。完整名称与路径可通过项目名的悬停提示辨认，不在操作菜单中展示。
- 状态在“操作”之后、用量之前，保留文字，不只靠颜色。正常连接时只显示“运行中/空闲/等待审批”等任务状态，不额外重复“已连接”；断线/同步失败优先，完整组合保留在状态的可访问名称与悬停提示中。
- “操作”使用文字加下拉箭头。浮层**只有两项：刷新、释放**，不显示标题、项目、路径、会话 ID 或状态等信息块。**两个动作使用完全一致的菜单行样式**：同宽、同高、同内边距、同字号、同文字颜色、同 hover/focus 背景及同禁用样式；不再一个有边框按钮、一个 quiet 文字按钮。菜单按两行内容收紧，不保留原信息区的空白。“刷新”沿用当前会话刷新；“释放”沿用原“释放并关闭”的行为，确认占用已释放后关闭会话页。仅缩短文案，不改变现有调用、禁用条件和反馈，不增加归档、删除或新确认流程。
- 浮层通过点击外部、Esc 或再次点击入口关闭；切换会话、选中操作时也关闭。刷新进行中的反馈仍在入口/状态处可见，避免菜单关闭后看不到进度。操作下拉不触发项目 Git fetch。

### 底部：输入区向下收起

2026-09-22 用户验收后修订：收起态改为**紧凑输入栏**。以下规则已按修订更新，替代原先仅提供展开入口的恢复栏。

- 在附件操作行右端增加向下箭头，名称“收起输入区”，与回形针“附件”按钮同一行。附件提示优先压缩或在手机隐藏，保证收起按钮始终可见；不增加一整行工具栏。
- 收起附件列表、模型/推理/权限选项及快捷键提示；紧凑栏左侧保留回形针按钮，点击打开原有文件选择器。保留同一个文本框，紧凑态为一行高度、多行内容内部滚动；展开后恢复原来的输入框高度。
- 文本框右侧保留发送/插话、中止及向上箭头“展开输入”入口；紧凑态附件和展开图标按钮为 36×44px，保留可访问名称及悬停提示，其余操作按钮至少 44px。输入框使用 24px 行高，避免手机单行文字触发多余滚动条，多行内容仍可滚动。发送仍受占用、未知提交结果、上传未完成等原有条件限制，插话沿用当前轮次模型和权限。手机回车换行、点击按钮发送；桌面 Enter 发送、Shift+Enter 换行；输入法组字不发送。
- 有附件时在紧凑输入栏下方显示数量及上传/失败摘要，展开后管理附件。紧凑态可以通过回形针新增附件，粘贴/拖放上传仍仅在展开态启用。手机展开态顶部内边距为 4px，附件行与文本框间距为 4px，附件/收起按钮保持 44px 点击区域并居中对齐。异常详情及待处理定位入口保持。桌面主动展开后聚焦文本框，手机不主动唤起键盘，点击紧凑文本框可以直接输入。
- 折叠只是隐藏附加控件，不能重建文本框、卸载附件组件、终止上传、清空草稿、改变权限或自动发送消息。折叠时关闭内部权限等浮层，并把焦点移至“展开输入”；隐藏控件不能继续获得 Tab 焦点。
- “重新编辑”失败消息时，作为明确的编辑动作展开输入区并恢复原文。收起态不增加全局粘贴/拖放上传：保留在已展开的输入区域内处理，避免阅读时误触上传。

### 异常、审批与滚动

- 错误、断线、占用、释放/重试提示不能直接放进被隐藏的正文。展开态继续显示现有完整提示；收起态在恢复栏上方保留紧凑状态提示，带“查看详情”入口，点击后展开输入区查看原文和原有重试操作。没有提示时不占高度。
- 多条提示并存时显示最高优先级摘要：提交结果未知/操作失败、占用或占用未确认、同步/连接异常、释放或重试中；标记其他提示数量，展开后全部可读。不把折叠等同于关闭或确认错误，也不自动重试写操作。
- 待审批/待用户输入的原有卡片继续位于消息区，不随输入区隐藏。顶部状态持续显示“等待审批/等待输入”，收起输入区时提供“查看待处理”入口定位卡片，不弹开输入区、不自动提交回答。
- 对话区占满删除原标题区及输入区收起释放的高度，保留独立滚动。正在跟随最新消息时保持底部；阅读历史时尽可能保留当前首条可见消息及其视口偏移，到内容边界时允许自然限位，不跳到最后一条。
- 仅输入区收起偏好在当前页面内切换会话时沿用，刷新后输入区默认展开。只记录一个界面布尔值，不写入服务端、SQLite 或会话历史，不新增逐会话持久化；草稿仍沿用原机制，本文不承诺刷新后新增保存能力。

### 手机与可访问性

- 延续当前断点；在中间栏变窄（包括桌面同时打开两侧栏）时，项目名限制最大宽度并省略，标题使用剩余宽度并省略；两个字段不得挤走右侧控件。顶栏始终单行，不增加第二行；窄屏压缩间距，状态使用短文案，操作入口保留“操作”文字，可省略装饰箭头。完整文本保留在可访问名称中，手机也不向操作菜单添加信息区。
- 底部收起/展开与两端侧栏按钮触摸区域至少 44×44px；顶部没有会话折叠按钮。不使用横向滚动承载主要操作。
- 按钮支持 Enter/Space，具有 `aria-expanded`、`aria-controls` 和可见焦点。菜单复用现有原生 popover 与按钮习惯，支持键盘进出和返回触发点；不嵌套按钮，不将整行包成另一个按钮。
- 保持当前配色，用低强调箭头而非大片绿色按钮表示折叠。第一版不做高度动画，减少内容抖动；底部恢复栏位于正常布局中，不覆盖最后一条消息，并考虑手机安全区。

### 确认后的最小实施范围与验收

现有 `SessionView.tsx` 同时持有会话标题、状态、动作、草稿和滚动状态；`Attachments.tsx` 在卸载时会取消上传，因此必须保留挂载。优先在这些现有组件与 `style.css` 中调整布局，复用已有浮层方式；为 `PaneToolbar` 增加可选会话内容区域，由会话页传入项目名、标题、现有动作与状态，非会话调用保持当前外观。左右面板逻辑和后端接口不改。仅在现有布局状态中补充输入区收起布尔值，不创建顶部开合状态、新状态框架或依赖。

验收要求：

1. 桌面、390px 手机、桌面中栏窄宽度：输入区展开和收起均无横向溢出；固定顶栏的项目名、标题、操作、状态、用量顺序正确；没有旧标题区或顶部折叠按钮，消息区紧贴顶栏，用量与两端侧栏按钮没有被挤走。
2. 长标题、Windows/Linux 路径、同名目录、无项目、加载中和不可用会话的布局正确；宽屏有空间时标题全文可见，缩窄才自然省略，重新变宽恢复全文，悬停始终可见完整标题，不修改原生标题或项目；操作菜单只有“刷新”和“释放”，没有信息块或遗留空白，两个菜单项的默认、悬停、焦点及禁用样式一致。
3. 菜单外部点击/Esc/选中动作/切换会话均关闭；刷新和释放沿用原调用，不附带 fetch 或重复请求。
4. 草稿、附件、上传中状态、模型/权限选择经收起/展开保留；无多余网络请求、上传中断或隐藏控件获得焦点。
5. 运行中可中止；断线、占用、失败、提交结果未知、待审批均有可见入口，隐藏输入区不会导致误发送或隐瞒异常。
6. 历史阅读位置与底部跟随分别验证；流式更新、点击重新编辑、手机软键盘、左右抽屉不破坏折叠状态。
7. 既有发送/插话、附件、审批和会话切换回归通过；完成 TypeScript、生产构建及桌面/手机浏览器检查。实际执行结果与限制以验收记录为准。

实现复用 `PaneLayout` 的页面级状态、原生 popover 和现有会话动作；未新增依赖或后端接口。构建与验证不重启运行中的服务。
...
### 首页沿用最近生效选项（2026-09-23）

**状态：已实施。** 复用已有会话恢复后的原生结果，为首页新任务回填模型、推理强度和权限。首页不为读取偏好额外恢复旧会话，不写 Codex 全局配置。

- 会话页接受原生快照/增量后的有效状态时，记住最近确认的模型、推理强度及可复用权限预设；仅浏览历史、外部占用、释放及未知状态不覆盖记录。未发送的选择、失败或结果未知的提交不作为生效依据。
- 当前浏览器以 `codex-web:turn-settings:v1` 保存一份最小偏好，仅包含上述三个字段，不保存会话内容、目录、权限对象或凭据。刷新后可恢复；不同浏览器/访问地址各自独立。存储损坏或不可用时降级为原生默认值，不阻断任务。
- 首页已有明确选项的草稿优先，否则使用最近生效偏好。回填的是实际提交选项，不只是占位文字；用户可以继续修改或清除。新建项目和 Clone 使用同一套选项。
- 无偏好或用户清除覆盖时，复用权限目录已有的 `config/read(cwd)` 返回模型和推理默认字段；配置未指定时使用 `model/list` 的模型及推理默认提示。切换项目重新读取，保留用户显式选择，不让旧请求覆盖新项目。
- 权限仅复用已确认的标准预设。包含额外可写目录、不同网络/临时目录规则或其他策略的自定义权限不复制到新项目，改为沿用目标项目原生配置；不把自定义策略简化为更宽松的预设。可用性仍由原生要求与服务端校验决定。
- 已失效的模型/推理选项保留并标记待核对，受限权限仍显示不可用原因，用户重新选择后再提交；不静默替换成另一模型或更高权限。

验收：已有会话 → 首页 → 刷新 → 创建请求三项一致；未发送修改不污染记录；无记录及切换项目显示原生默认值；损坏/受限存储可用；自定义和未知权限不被错误继承；已有会话的当前值展示保持不变。

实现复用 `TurnOptions`、会话快照/SSE 和现有权限目录接口，无新依赖或原生写配置接口。运行时补齐 start/resume 返回及成功 turn/start 的推理强度同步，模型与推理以已确认状态优先，避免旧历史头覆盖刚生效的选择。仅换模型而未指定推理强度时清除旧强度的确认状态，不把旧模型的强度存入新模型偏好。首页自动回填不作为手动草稿覆盖记录。

2026-09-23 验证：`npm test` 246 通过、5 跳过（4 项 Linux、1 项独立便携包）；`npm run build` 通过。Playwright 在 Windows Edge、1440×900 / 390×844 下通过新增首页链路、已有会话选项 HTML 回归和原有 6 项工作区用例。独立审查后补充换模型时清除未确认强度，相关 Runtime、快照/增量及偏好测试 110 项再次全部通过，重新构建通过。浏览器使用模拟 API，验证显示、存储、刷新、导航与实际请求载荷；原生接口路径由 Runtime fixture 覆盖，本轮未运行真实模型任务、修改全局配置或重启当前业务服务。受限环境的临时缓存/测试进程清理问题通过工作区临时目录和授权的完整测试运行解决。
