...
![固定单行顶栏：项目名与短标题、统一操作菜单、标题悬停全文，以及输入区展开和收起示意](../assets/session-toolbar-v4.png)

[查看第四版原图](../assets/session-toolbar-v4.png)。此图替代前三版的顶部设计：上图展示输入区展开及操作菜单，下图展示输入区收起及短标题悬停全文。两种状态都只有同一条固定顶栏，没有顶部展开态，也没有顶部折叠按钮；项目名、标题、操作、运行状态、用量依次排列，两端保留侧栏开关。
...
会话页以项目名称替换顶栏左侧固定的“Codex Web”文字；品牌仍由左侧导航保留。顶栏从左至右严格为：**左侧栏开关 → 项目名 → 会话短标题 → 操作下拉 → 运行/连接状态 → 用量入口 → 右侧栏开关**。非会话页面的顶栏不在本次修改范围。

顶栏使用现有高度，手机触摸区域不低于 44px。原标题/项目区直接删除，不保留第二行、空白占位或额外边框，对话内容紧接顶栏下沿。没有顶部展开按钮；操作菜单的小箭头只用于打开菜单，不能恢复旧标题区。

- 标题随可用宽度单行省略，采用末尾省略号，不按固定字符数裁剪，不让路径样式的长标题撑宽。**鼠标悬停短标题时必须显示完整标题**，优先使用原生 `title` 提示，不新增 tooltip 依赖。完整标题保留在 DOM 文本和可访问名称中，不在“操作”内重复展示。标题本身不作为展开或导航按钮。
...
2. 长标题、Windows/Linux 路径、同名目录、无项目、加载中和不可用会话的布局正确；悬停短标题可见完整标题，不修改原生标题或项目；操作菜单只有“刷新”和“释放”，没有信息块或遗留空白，两个菜单项的默认、悬停、焦点及禁用样式一致。
