...
图片由内置图像生成工具基于脱敏第三版修订，用于确认布局，不是已实现页面截图。图中的标题、项目名、对话、文件名、进度和用量均为虚构演示数据，不使用真实会话内容。生成约束：顶部固定单行，项目名替代固定品牌文字，操作在运行状态之前；菜单内两个动作采用同一视觉样式；悬停短标题展示完整标题。图片仅示意布局与交互，具体间距及异常状态以本节规格为准。
...
- 标题随可用宽度单行省略，采用末尾省略号，不按固定字符数裁剪，不让路径样式的长标题撑宽。**鼠标悬停短标题时必须显示完整标题**，优先使用原生 `title` 提示，不新增 tooltip 依赖。完整标题同时保留在可访问名称及“操作”浮层的会话信息中，手机可点操作查看全文，不依赖悬停。标题本身不作为展开或导航按钮。
- 项目使用项目显示名；缺少名称时取目录末级名称，同时支持 Windows 和 Linux 分隔符。无可用项目时显示“未关联项目”，不把会话 ID 当项目名。完整路径只在会话信息内展示并允许选择复制，目录同名时可据此辨认。
- 状态在“操作”之后、用量之前，保留文字，不只靠颜色。正常连接时只显示“运行中/空闲/等待审批”等任务状态，不额外重复“已连接”；断线/同步失败优先，完整组合在会话信息内可查看。
- “操作”使用文字加下拉箭头。浮层顶部为只读完整标题、项目名、完整路径、会话 ID 和完整状态，下方依次为“刷新当前会话”和“释放并关闭”。**两个动作使用完全一致的菜单行样式**：同宽、同高、同内边距、同字号、同文字颜色、同 hover/focus 背景及同禁用样式；不再一个有边框按钮、一个 quiet 文字按钮。信息与动作之间可分隔，两动作之间不做视觉等级区别。复用现有调用、禁用条件和反馈，不增加归档、删除或新确认流程。
...
- 延续当前断点；在中间栏变窄（包括桌面同时打开两侧栏）时，项目名限制最大宽度并省略，标题使用剩余宽度并省略；两个字段不得挤走右侧控件。顶栏始终单行，不增加第二行；窄屏压缩间距，状态使用短文案，操作入口保留“操作”文字，可省略装饰箭头。完整项目名、标题及状态仍可从操作浮层查看。
...
2. 长标题、Windows/Linux 路径、同名目录、无项目、加载中和不可用会话的布局正确；悬停短标题可见完整标题，触屏可在操作浮层查看，不修改原生标题或项目；两个操作菜单项的默认、悬停、焦点及禁用样式一致。
