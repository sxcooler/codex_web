...
| 方案 | 取舍 |
| --- | --- |
| 上下独立收起，各保留单行恢复栏（推荐） | 可以只缩顶部、只缩输入区或同时缩；状态和恢复入口位置稳定 |
| 顶部信息全部并入应用工具栏 | 少一行，但会与左右栏开关、用量入口争夺窄屏空间 |
| 上下区域完全隐藏，仅留悬浮按钮 | 节省最多空间，但不易发现恢复入口，容易遗漏运行状态和草稿 |

采用第一种方案。首次进入上下均展开，但展开态本身也更紧凑；不随滚动、发送、任务完成或新消息自动收起/展开。两个开关互不联动，也不影响左右侧栏及消息内“执行过程”折叠。

### 顶部：紧凑信息区与向上收起

从左至右是会话摘要、状态、操作下拉、收起按钮。右端控件不被长标题挤出屏幕。

| 状态 | 内容与尺寸目标 |
| --- | --- |
| 展开 | 第一行：单行标题、连接/任务状态、操作下拉、向上箭头；第二行：项目名称。桌面约 76–88px 高，标题约 18px，不再用大标题撑高 |
| 收起 | 一行短标题、状态、操作下拉、向下箭头；项目行隐藏。桌面约 44px，手机不低于 48px |
...
- 右端箭头有明确名称“收起会话信息”/“展开会话信息”；只点按钮切换，不让整块标题成为开关，避免选中文字时误触。
...
- 延续当前断点；在中间栏变窄（包括桌面同时打开两侧栏）时，也必须省略标题并保留控件。若状态和标题无法共用一行，展开态允许状态移至项目行，收起态保留优先状态，标题使用剩余宽度。
...
现有 `SessionView.tsx` 同时持有会话标题、状态、动作、草稿和滚动状态；`Attachments.tsx` 在卸载时会取消上传，因此必须保留挂载。优先在这些现有组件与 `style.css` 中调整布局，复用已有浮层方式；顶部应用 `PaneToolbar`、左右面板逻辑和后端接口不改。仅当需要让两项偏好跨会话保留时，在现有布局状态中补充布尔值，不创建新的状态框架或依赖。

确认后验证：

1. 桌面、390px 手机、桌面中栏窄宽度：四种上下组合均无横向溢出；标题省略、项目短名、完整信息入口、操作菜单均可用。
