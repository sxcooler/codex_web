...
## 会话页上下收起与紧凑布局（2026-09-22，待用户确认）

**状态：仅完成设计，尚未实施。** 本节以用户提供的会话页标注截图为依据；红框指会话信息区，黄框指输入区右上角入口，绿框指输入区。色框不进入正式界面。确认后，本节替代此前相关标题、路径及按钮布局规定，其他流程保持现有行为。

### 当前原型图（第三版）

![会话页四种状态：全部展开、顶部收进现有顶栏、收起输入区、全部收起](../assets/session-collapse-v3.png)

[查看第三版原图](../assets/session-collapse-v3.png)。此图替代前两版：右列的顶部收起态仅保留原应用顶栏，短标题位于左侧栏开关与用量之间，下面的标题/项目区域完全消失；左列展开态在顶栏下显示标题和项目名。四个窗口等高，底部收起独立控制。

图片由内置图像生成工具基于第二版修订，用于确认布局，不是已实现页面截图。图中的标题、项目名、对话、文件名、进度和用量均为虚构演示数据，不使用真实会话内容。生成约束：保留深色风格和四种状态，移除会话顶栏固定“Codex Web”，将状态/操作/收起按钮放入原顶栏；收起时短标题也并入同一行，不留第二条标题栏。文字、具体间距及异常状态以本节规格为准。
...
| 方案 | 取舍 |
| --- | --- |
| 上下独立收起，各保留单行恢复栏（原方案，不采用） | 顶部收起后仍多占一行，不符合用户对释放高度的预期 |
| 顶部收进现有应用工具栏，底部保留单行恢复栏（修订采用） | 移除会话页固定“Codex Web”文字，用其与用量入口之间的空间承载收起摘要，标题区完整退出布局 |
| 上下区域完全隐藏，仅留悬浮按钮 | 节省最多空间，但不易发现恢复入口，容易遗漏运行状态和草稿 |

根据用户看过原型图后的反馈，采用第二种方案。此前两张原型图的顶部收起态作废，底部交互仍可参考。首次进入上下均展开，但展开态本身也更紧凑；不随滚动、发送、任务完成或新消息自动收起/展开。两个开关互不联动，也不影响左右侧栏及消息内“执行过程”折叠。

### 顶部：紧凑信息区与向上收起

会话页移除应用顶栏左侧固定的“Codex Web”文字；品牌仍由左侧导航保留。原顶栏从左至右为：左侧栏开关、弹性摘要区域、状态、操作下拉、展开/收起按钮、用量入口、右侧栏开关。状态、操作和展开/收起按钮始终留在同一顶栏位置；用量入口和两端侧栏开关保持可用。非会话页面的顶栏不在本次修改范围。

| 状态 | 内容与尺寸目标 |
| --- | --- |
| 展开 | 顶栏下方显示单行标题与项目名称两行信息，约 60–72px 高，标题约 18px；顶栏摘要区域留白，不重复标题，顶栏按钮为向上箭头 |
| 收起 | 顶栏下方的信息区完全隐藏，高度为 0，不保留摘要条、空白占位或边框。短标题移到现有顶栏的弹性摘要区域，顶栏按钮改为向下箭头；对话内容紧接顶栏下沿 |

- 标题随可用宽度单行省略，采用末尾省略号，不按固定字符数裁剪，不让路径样式的长标题撑宽。完整标题保留在可访问名称和“操作”浮层的会话信息中；桌面可同时提供悬停提示，手机不依赖悬停。
- 项目使用项目显示名；缺少名称时取目录末级名称，同时支持 Windows 和 Linux 分隔符。无可用项目时显示“未关联项目”，不把会话 ID 当项目名。完整路径只在会话信息内展示并允许选择复制，目录同名时可据此辨认。
- 状态保留文字，不只靠颜色。桌面沿用“已连接 · 运行中”等完整组合；窄屏显示最重要的任务状态，断线/同步失败优先，完整组合在会话信息内可查看。
- 顶栏箭头有明确名称“收起会话信息”/“展开会话信息”；只点按钮切换，不让整块标题成为开关，避免选中文字时误触。收起后恢复入口不随信息区消失。
- “操作”使用文字加下拉箭头。浮层顶部为只读完整标题、项目名、完整路径、会话 ID 和完整状态，下方是“刷新当前会话”和“释放并关闭”；释放项与刷新分隔。复用现有动作、禁用条件和反馈，不增加归档、删除或新确认流程。
- 浮层通过点击外部、Esc 或再次点击入口关闭；切换会话、收起信息区、选中操作时也关闭。刷新进行中的反馈仍在入口/状态处可见，避免菜单关闭后看不到进度。操作下拉不触发项目 Git fetch。
...
- 对话区占满上下收起释放的高度，保留独立滚动。正在跟随最新消息时保持底部；阅读历史时尽可能保留当前首条可见消息及其视口偏移，到内容边界时允许自然限位，不跳到最后一条。
- 两个偏好在当前页面内切换会话时沿用，刷新后默认展开。只记录两个界面布尔值，不写入服务端、SQLite 或会话历史，不新增逐会话持久化；草稿仍沿用原机制，本文不承诺刷新后新增保存能力。

### 手机与可访问性

- 延续当前断点；在中间栏变窄（包括桌面同时打开两侧栏）时，也必须省略标题并保留控件。顶栏始终单行，不因收起而新增第二行；优先压缩间距、缩短状态文案并将操作入口改为更多图标，标题使用剩余宽度。隐藏正常“已连接”赘述，保留运行/等待/异常等有意义状态；完整标题及完整状态仍可从操作浮层查看。
- 收起/展开触摸区域至少 44×44px；顶部窄屏操作可缩为带可访问名称的更多图标，仍单独保留收起按钮。不使用横向滚动承载主要操作。
...
现有 `SessionView.tsx` 同时持有会话标题、状态、动作、草稿和滚动状态；`Attachments.tsx` 在卸载时会取消上传，因此必须保留挂载。优先在这些现有组件与 `style.css` 中调整布局，复用已有浮层方式；为 `PaneToolbar` 增加可选会话内容区域，由会话页传入标题、状态和现有动作，非会话调用保持当前外观。左右面板逻辑和后端接口不改。仅当需要让两项偏好跨会话保留时，在现有布局状态中补充布尔值，不创建新的状态框架或依赖。

确认后验证：

1. 桌面、390px 手机、桌面中栏窄宽度：四种上下组合均无横向溢出；标题省略、项目短名、完整信息入口、操作菜单均可用。顶部收起后仅剩原应用顶栏，原标题区高度为 0，消息区紧贴顶栏；用量与两端侧栏按钮没有被挤走。
2. 长标题、Windows/Linux 路径、同名目录、无项目、加载中和不可用会话的布局正确；折叠不修改原生标题或项目。
