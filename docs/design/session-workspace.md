...
## 会话页固定顶栏与输入区收起（2026-09-22，待用户确认）

**状态：仅完成设计，尚未实施。** 本节以用户提供的会话页标注截图为依据；红框指会话信息区，黄框指输入区右上角入口，绿框指输入区。色框不进入正式界面。确认后，本节替代此前相关标题、路径及按钮布局规定，其他流程保持现有行为。
...
7. 既有发送/插话、附件、审批和会话切换回归通过；完成 TypeScript、生产构建及桌面/手机浏览器检查。上述是待执行验收，不能视为已通过。

本轮仅修改本文及文档索引，等待用户确认后实施，不改运行中的服务。
