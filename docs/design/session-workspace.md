...
- 在附件操作行右端增加向下箭头，名称“收起输入区”，与“＋ 附件”同一行。附件提示优先压缩或在手机隐藏，保证收起按钮始终可见；不增加一整行工具栏。
- 收起附件操作/列表、模型/推理/权限选项及快捷键提示。保留同一个文本框，紧凑态为一行高度、多行内容内部滚动；展开后恢复原来的输入框高度。
...
- 有附件时在紧凑输入栏下方显示数量及上传/失败摘要，展开后管理附件。异常详情及待处理定位入口保持。桌面主动展开后聚焦文本框，手机不主动唤起键盘，点击紧凑文本框可以直接输入。
