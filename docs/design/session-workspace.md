...
图片由内置图像生成工具基于脱敏第三版修订，用于确认布局，不是已实现页面截图。图中的标题、项目名、对话、文件名、进度和用量均为虚构演示数据，不使用真实会话内容。生成约束：顶部固定单行，项目名替代固定品牌文字，操作在运行状态之前；菜单只包含同一视觉样式的“刷新”和“释放”，没有会话信息块；悬停短标题展示完整标题。图片仅示意布局与交互，具体间距及异常状态以本节规格为准。
...
- 收起范围包括附件操作/列表、文本框、模型/推理/权限选项、快捷键提示及发送/插话按钮。下方保留约 44px（手机至少 48px）的恢复栏。
- 恢复栏左侧显示“输入已收起”；有内容时追加“有草稿 · N 个附件”，上传中/失败时追加相应摘要。只显示计数和状态，不展示草稿正文。右侧为“展开输入”加向上箭头；任务活跃时在其左侧保留“中止”，中止中显示原有忙碌状态。
- 不在收起态提供直接发送，避免看不到草稿时误发。展开后恢复原有内容、附件、设置和输入框高度；桌面主动点击“展开输入”后聚焦输入框，手机只展开、不自动弹出软键盘。
- 折叠只是隐藏，不能卸载附件组件、终止上传、清空草稿、改变权限或发送消息。折叠时关闭内部权限等浮层，并把焦点移至“展开输入”；隐藏控件不能继续获得 Tab 焦点。
