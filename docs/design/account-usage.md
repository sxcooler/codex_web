...
- 入口的圆角色块高 32px，在 44px 顶栏内上下各留 6px，按钮点击区域仍至少 44×44px。有效额度用连续色相表达剩余比例：0% 为红色，100% 为薄荷绿，中间平滑过渡；文字、深色底和描边使用同一色相。依据主额度最紧张周期计算，手机缩略入口沿用同样颜色。未知数据使用中性灰，失败/过期改为琥珀色并保留“待更新”文字，不以旧比例着色。
...
- 各周期的百分比文字与进度条按各自剩余比例使用与入口一致的红→绿连续渐变，不统一套用顶栏最小值；进度条长度仍表示剩余比例。
...
最初依据本机 Codex CLI 0.153.4 生成协议核对接口；2026-09-25 核对 0.156.1 仍提供相同的重置接口、幂等参数和四种结果。此前已成功只读获取账户额度和重置机会；**未调用真实消耗接口**。不以精确 CLI 版本白名单限制功能。
...
- 初始化没有重置能力清单；CLI 升级不等于接口失效，不按版本号禁用消耗。根据 ChatGPT 账户类型、可靠账户标识和原生返回的有效机会启用操作；用户确认后才调用原生接口，保留同键重试与结果待核实保护。
...
- 原生消耗协议没有原子的预期账户参数；提交前在同一个 app-server 实例再次读取并校验账户，然后立即发送明确的 `creditId`。不省略机会 ID，也不让原生服务自动挑选另一条机会。
...
- 首次提交遇到账户已变化或明确的 JSON-RPC `-32601`（接口不存在），清除本次未执行操作并报告原因；若重试的是此前结果未知的操作，则仍保留原幂等键，不能因当前拒绝而推断此前未执行。

### 2026-09-25 升级兼容修复验证

- 移除 0.153.4 精确版本白名单；模拟 0.156.1、预发布版本与未来版本均可走原生重置调用。
- 账户不匹配、明确缺少接口、超时未知、同键重试、并发去重、持久化恢复、认证与 CSRF 回归通过；未调用真实消耗接口。
- Windows Node 24.20.0 全量 259 项：254 通过、5 项环境条件跳过；桌面 1440px 与手机 390px 用量面板自测通过；类型检查和生产构建通过。
