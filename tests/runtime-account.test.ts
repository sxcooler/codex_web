...
test('account RPCs keep credentials private, allow CLI upgrades and recheck identity before consume',async t=>{
  ...
  for(const version of ['codex_remote_web/0.156.1','codex_remote_web/0.153.4-beta','codex_remote_web/0.200.0 (client 0.153.4)']){
    (runtime as any).initializeInfo.userAgent=version;
    assert.equal((await runtime.accountIdentity()).resetProtocol,true);
    assert.deepEqual(await runtime.consumeAccountReset(input,expected),{outcome:'reset'});
    assert.deepEqual(calls.at(-1),{method:'account/rateLimitResetCredit/consume',params:input});
  }
  ...
  missingMethod=true;
  await assert.rejects(runtime.consumeAccountReset(input,expected),{code:'RUNTIME_ACCOUNT_UNSUPPORTED'});
