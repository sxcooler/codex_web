...
export function Session({id,onChanged,onReleased}:{id:string;onChanged:()=>Promise<void>;onReleased:(result:Json)=>void}){
  ...
  return <div className={`session-layout ${snapshot?.project&&panes.rightVisible?'with-git':''}`}><div className="session-center"><PaneToolbar>
    ...
    <form className={'composer'+(collapsed?' is-collapsed':'')} onSubmit={e=>{e.preventDefault();void sendMessage();}}>
      ...
      <Attachments items={attachments} onChange={next=>{setAttachments(next);saveDraft(draft,settings,next);}} onError={setAttachmentError} disabled={busy} hidden={collapsed}><button type="button" className="quiet composer-collapse" aria-label="收起输入区" title="收起输入区" aria-expanded={!collapsed} aria-controls={composerId+' '+composerId+'-options'} onClick={()=>setCollapsed(true)}>⌄</button></Attachments>
      </div>
      ...
      {collapsed?<div className="composer-actions">{stopButton}{sendButton}<button ref={expandButton} type="button" className="quiet" aria-label="展开输入" title="展开附件和输入设置" aria-expanded={!collapsed} aria-controls={composerId+' '+composerId+'-options'} onClick={()=>setCollapsed(false)}>展开输入 <span aria-hidden="true">⌃</span></button></div>:null}
      </div>
