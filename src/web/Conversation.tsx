...
export function Pending({pending,items,respond,busy}:{pending:Json;items:Json[];respond:(answer:Json)=>Promise<void>;busy:boolean}) {
  ...
  return <section className="approval"><h3>{questions?'需要你的输入':mcp?'工具请求确认':'等待你的审批'}</h3><p>{p.reason??p.message??''}</p>
    ...
    {questions?<form onSubmit={e=>{e.preventDefault();void respond({answers:Object.fromEntries((p.questions??[]).map((q:Json)=>[q.id,{answers:[answers[q.id]??'']}]))});}}>{(p.questions??[]).map((q:Json)=><label key={q.id}>{q.question}{q.options?.length&&!q.isOther?<select required value={answers[q.id]??''} onChange={e=>setAnswers({...answers,[q.id]:e.target.value})}><option value="" disabled>请选择</option>{q.options.map((o:Json)=><option key={o.label} value={o.label}>{o.label} — {o.description}</option>)}</select>:<><input type={q.isSecret?'password':'text'} required value={answers[q.id]??''} onChange={e=>setAnswers({...answers,[q.id]:e.target.value})}/>{q.options?.length?<span className="muted small">选项：{q.options.map((o:Json)=>o.label).join(' / ')}，也可自行输入。</span>:null}</>}</label>)}<button className="primary" disabled={busy}>提交回答</button></form>:null}
