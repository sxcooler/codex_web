...
import { loadTurnSettings } from './turnSettings.ts';
...
function App() {
  ...
  useEffect(()=>{if(authenticated)void loadSessions();else{drafts.clear();++sessionsRequest.current;sessionsQuery.current=null;setSessions([]);setCursor(null);setSessionsLoading(false);setSessionsError('');}},[authenticated,loadSessions]);
...
function Settings({onChanged}:{onChanged:()=>void}) {
  ...
  return <main className="settings" inert={panes.mobile&&panes.drawer!==null}><h1>设置</h1><p className="muted">当前开发机与 Codex 运行时状态。</p><ErrorBox error={error}/>{settings?<><dl><dt>访问地址</dt><dd>{settings.origin}</dd><dt>Codex Web</dt><dd>{settings.appVersion??"未知"}</dd><dt>Node.js</dt><dd>{settings.nodeVersion}</dd><dt>工作根目录</dt><dd>{settings.workRoot}</dd>{settings.modelSource?<><dt>Codex CLI</dt><dd>{settings.modelSource.version??'未加载'}</dd><dt>CLI 路径</dt><dd>{settings.modelSource.executable}</dd><dt>模型目录读取</dt><dd>{settings.modelSource.readAt?new Date(settings.modelSource.readAt).toLocaleString():'尚未读取'}</dd></>:null}</dl><div className="actions"><button disabled={busy} onClick={async()=>{setBusy(true);setError('');try{await api('/models?refresh=true');setSettings(await api('/settings'));}catch(e:any){setError(e.message);}finally{setBusy(false);}}}>刷新模型列表</button><button disabled={busy} onClick={async()=>{setBusy(true);setError('');try{await api('/models/reload',{});setSettings(await api('/settings'));}catch(e:any){setError(e.message);}finally{setBusy(false);}}}>空闲时重载 Codex</button></div><p className="muted small">刷新读取当前 CLI 的模型目录；新模型仍不可见时，请检查独立 Codex CLI 的版本。重载不会升级 CLI，活动任务期间不可执行。</p><details><summary>Codex 诊断与有效权限</summary><pre>{pretty(settings.runtime)}</pre></details><p className="muted small">路径、端口与权限配置变更后需重启服务。</p></>:<p>读取诊断…</p>}<Notifications/><button onClick={panes.reset}>重置布局</button><hr/><h2>修改管理员密码</h2><p className="muted">修改后所有已登录设备需要重新登录。</p><form onSubmit={async e=>{e.preventDefault();setBusy(true);setError('');try{await api('/auth/password',{currentPassword:current,newPassword:next});setCurrent('');setNext('');onChanged();}catch(e:any){setError(e.message);}finally{setBusy(false);}}}><label>当前密码<input type="password" required minLength={12} maxLength={256} autoComplete="current-password" value={current} onChange={e=>setCurrent(e.target.value)}/></label><label>新密码<input type="password" required minLength={12} maxLength={256} autoComplete="new-password" value={next} onChange={e=>setNext(e.target.value)}/></label><button className="primary" disabled={busy}>{busy?'保存中…':'修改密码并退出'}</button></form></main>;
}
