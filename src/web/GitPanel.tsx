...
export function GitPanel({id,projectId,tick,visible}:{id:string;projectId:string;tick:number;visible:boolean}){
  ...
  return <aside id="right-sidebar" aria-label="项目面板" className="git-panel" hidden={!visible} ref={panelRef} onWheel={()=>{restoreScroll.current=null;}} onTouchStart={()=>{restoreScroll.current=null;}} onScroll={event=>{if(event.target!==event.currentTarget||restoreScroll.current!==null)return;const scroll=event.currentTarget.scrollTop;if(state.tab==='history')setState(current=>({...current,history:{...current.history,scroll}}));else updateSection(state.tab,{previewScroll:scroll});}}>
    ...
    <div hidden={state.tab==='history'}>
      ...
      <div className="file-list" ref={listRef} onScroll={event=>updateSection(state.tab as 'changes'|'files',{listScroll:event.currentTarget.scrollTop})}>{busy&&!files.length?<p>读取中…</p>:!files.length?<p className="muted">{state.tab==='changes'?'没有变更':'目录为空'}</p>:files.map(file=><button key={file.path} className={path===file.path?'active':''} title={file.oldPath?file.oldPath+' → '+file.path:file.path} onClick={()=>{if(file.type==='directory')updateSection('files',{directory:file.path,path:''});else updateSection(state.tab as 'changes'|'files',{path:file.path});}}><span>{file.type==='directory'?'▸ ':''}{file.path}</span><small>{file.status??''}{file.binary?' 二进制':file.added!==undefined?' +'+file.added+' −'+file.deleted:''}</small></button>)}</div>
