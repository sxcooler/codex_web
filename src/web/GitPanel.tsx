...
export function GitPanel({id,projectId,tick,visible,openFile}:{id:string;projectId:string;tick:number;visible:boolean;openFile?:{path:string}}){
  ...
  return <aside id="right-sidebar" aria-label="项目面板" className="git-panel" hidden={!visible}>
    ...
    <div className="history-panel" hidden={state.tab!=='history'}><GitHistory id={id} tick={tick+revision+historyRevision} visible={visible&&state.tab==='history'} state={state.history} onChange={history=>setState(current=>({...current,history:{...current.history,...history}}))} onScroll={rememberHistory}/></div>
    <div className="panel-sections" data-collapsed={section.collapsed} hidden={state.tab==='history'}>
      ...
      <div className="file-list" ref={listRef} onScroll={event=>visible&&!section.collapsed&&rememberSection(state.tab as 'changes'|'files',{listScroll:event.currentTarget.scrollTop})}>{busy&&!files.length?<p>读取中…</p>:!files.length&&error?null:!files.length?<p className="muted">{state.tab==='changes'?'没有变更':'目录为空'}</p>:files.map(file=><button key={file.path} className={path===file.path?'active':''} title={file.oldPath?file.oldPath+' → '+file.path:file.path} onClick={()=>{if(file.type==='directory')updateSection('files',{directory:file.path,path:''});else updateSection(state.tab as 'changes'|'files',{path:file.path});}}><span>{file.type==='directory'?'▸ ':''}{file.path}</span><small>{file.status??''}{file.binary?' 二进制':file.added!==undefined?' +'+file.added+' −'+file.deleted:''}</small></button>)}</div>
      </div>
      </div>
