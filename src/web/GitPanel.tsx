...
export function GitPanel({id,projectId,tick,visible}:{id:string;projectId:string;tick:number;visible:boolean}){
  ...
  useLayoutEffect(()=>{if(!visible||state.tab==='history')return;if(listRef.current)listRef.current.scrollTop=section.listScroll;if(previewRef.current)previewRef.current.scrollTop=section.contentScroll;},[visible,state.tab,listKey,files,contentKey,content,state.files.mode]);
  ...
  return <aside id="right-sidebar" aria-label="项目面板" className="git-panel" hidden={!visible}>
    ...
    <div className="history-panel" hidden={state.tab!=='history'}><GitHistory id={id} tick={tick+revision+historyRevision} visible={visible&&state.tab==='history'} state={state.history} onChange={history=>set({history})}/></div>
    <div className="panel-sections" hidden={state.tab==='history'}>
