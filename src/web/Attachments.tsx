...
export function Attachments({items,onChange,disabled=false,hidden=false,children,onError}:{items:Attachment[];onChange:(items:Attachment[])=>void;disabled?:boolean;hidden?:boolean;children?:ReactNode;onError?:(error:string)=>void}){
  ...
  useEffect(()=>{const form=root.current?.closest('form');if(!form||hidden)return;
  ...
  return <div ref={root} className="attachments">
    <input hidden ref={input} type="file" multiple onChange={e=>{add([...(e.target.files??[])]);e.target.value='';}}/><button type="button" disabled={disabled} onClick={()=>input.current?.click()}>＋ 附件</button><span className="muted small attachment-hint">拖入文件或在此粘贴图片</span>{children}
